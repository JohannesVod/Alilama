...
To test your trained model, see the [inference](inference.ipynb) notebook. Specify the start string and execute the model on it. You can see the already trained models in the models folder. I already trained "alilama_26M.pth" (d_model=480, blocks=8, max_seq_len=128, num_heads=8, hidden_dim=4*d_model) with learning rate 1e^-4. It took around 20 hours to train on my NVIDIA GeForce RTX 3050, but the results were already decent after 10 hours. Training for even longer probably would have given even better results.
